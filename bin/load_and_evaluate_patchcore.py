...
@click.argument("threshold_path", type=str)
...
def run(methods, results_path, gpu, seed, save_reports, threshold_path):
    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        with device_context:
            ...
            if dataloader_count < n_patchcores:
                PatchCore_list = next(iter(patchcore_iter))
            ...
            if save_reports:
                reports_save_path = os.path.join(
                    results_path, "reports", dataset_name
                )
                os.makedirs(reports_save_path, exist_ok=True)
            
            optimal_threshold = float(pd.read_csv(threshold_path+"/results.csv", index_col=0).loc[dataset_name, 'optimal_threshold'])
